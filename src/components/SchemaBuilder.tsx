...
export default function SchemaBuilder({ 
  ...
  const generatePreview = useMemo((): Record<string, unknown> => {
    const generateFieldPreview = (field: SchemaField): unknown => {
      switch (field.type) {
        ...
        case 'email':
        case 'url':
          return field.logic?.enum ? `enum: [${field.logic.enum.join(', ')}]` : 'string';
        case 'number':
          return field.logic?.enum ? `enum: [${field.logic.enum.join(', ')}]` : 'number';
        case 'boolean':
          return 'boolean';
        case 'date':
          return 'date';
        ...
        case 'object':
          ...
          if (collapsedFields.has(field.id) && field.children && field.children.length > 0) {
            return `{ ...${field.children.length} field${field.children.length !== 1 ? 's' : ''} }`;
          }
    ...
    if (arrayItem.type === 'object') {
      return (
        <Box key={`${arrayItem.id}-arrayitem`}>
          ...
          {arrayItem.children && arrayItem.children.length > 0 && 
            renderTreeItems(arrayItem.children, [...parentPath, 'arrayItem'], level + 1)}
        </Box>
      );
      ...
      return (
        <Box key={`${arrayItem.id}-nestedarray`}>
                ...
                onChange={(e) => {
                  ...
                  const updatedArrayItem = {
                    ...
                    arrayItemType: {
                      ...
                      type: newItemType,
                      children: newItemType === 'object' ? arrayItem.arrayItemType?.children || [] : undefined,
                      arrayItemType: newItemType === 'array' ? {
                        id: uuidv4(),
                        name: 'item',
                        type: 'text',
                        children: undefined,
                      ...
                      logic: arrayItem.arrayItemType?.logic || { required: false }
                    } as SchemaField,
                  };
                  ...
                  setSchema(prev => ({
                    ...prev,
                    fields: findAndUpdateField(prev.fields, arrayItem.id, () => updatedArrayItem)
                  }));
                }}
                displayEmpty
                sx={{ 
          ...
          {renderArrayItemTree(arrayItem.arrayItemType, [...parentPath, 'arrayItem'], level + 1)}
        </Box>
      );
    }
    return null;
  ...
  const renderTreeItems = (fields: SchemaField[], parentPath: string[] = [], level: number = 0) => {
    return fields.map((field) => {
      ...
      const childrenCount = field.type === 'object' ? (field.children?.length || 0) : 
                           (field.type === 'array' && field.arrayItemType ? 1 : 0); // Arrays always show "1 item" when collapsed
        ...
        <Box key={field.id}>
          ...
          >
            ...
            {hasChildren ? (
              <IconButton
                ...
                sx={{ p: 0.25 }}
              >
                {isCollapsed ? <ArrowRightIcon fontSize="small" /> : <ArrowDownIcon fontSize="small" />}
              </IconButton>
            ) : (
            ...
            <FormControl size="small" sx={{ minWidth: 80 }}>
              <Select
                ...
                sx={{ 
                  ...
                  '& .MuiSelect-select': { py: 0.25, px: 1 },
                  '& .MuiOutlinedInput-notchedOutline': { border: 'none' },
                  backgroundColor: parentPath.length > 0 ? "grey.100" : "primary.50",
                  borderRadius: 1,
                  minHeight: '20px'
                }}
                onClick={(e) => e.stopPropagation()}
            ...
            {hasChildren && isCollapsed && (
              <Chip
                label={`${childrenCount} field${childrenCount !== 1 ? 's' : ''}`}
                size="small"
                color="info"
                variant="outlined"
                sx={{ fontSize: '0.65rem', height: '18px' }}
          ...
          {field.type === 'array' && field.arrayItemType && !isCollapsed && (
            <Box key={`${field.id}-arrayitem`}>
              ...
              >
                ...
                {(field.arrayItemType?.type === 'object' && field.arrayItemType?.children && field.arrayItemType.children.length > 0) || 
                 (field.arrayItemType?.type === 'array' && field.arrayItemType?.arrayItemType) ? (
                  <IconButton
                    size="small"
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    sx={{ p: 0.25 }}
                  >
                    {collapsedFields.has(field.arrayItemType!.id) ? <ArrowRightIcon fontSize="small" /> : <ArrowDownIcon fontSize="small" />}
                  </IconButton>
                ) : (
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      const updatedArrayItem = {
                        ...
                        type: newItemType,
                        children: newItemType === 'object' ? field.arrayItemType?.children || [] : undefined,
                        arrayItemType: newItemType === 'array' ? {
                          id: uuidv4(),
                          name: 'item',
                          type: 'text',
                        ...
                        logic: field.arrayItemType?.logic || { required: false }
                      } as SchemaField;
                ...
                {((field.arrayItemType?.type === 'object' && field.arrayItemType?.children && field.arrayItemType.children.length > 0) || 
                  ...
                  <Chip
                    label={field.arrayItemType?.type === 'object' 
                      ? `${field.arrayItemType.children?.length || 0} field${(field.arrayItemType.children?.length || 0) !== 1 ? 's' : ''}`
                      : '1 nested array'
                    }
                    size="small"
                    color="info"
              ...
              {field.arrayItemType?.type === 'object' && field.arrayItemType?.children && field.arrayItemType.children.length > 0 && !collapsedFields.has(field.arrayItemType.id) && 
                renderTreeItems(field.arrayItemType.children, [...parentPath, field.id, 'arrayItem'], level + 2)}
            </Box>
          )}
          
          {/* Render nested array items recursively - only when parent array is expanded and nested array is expanded */}
          {field.type === 'array' && field.arrayItemType && field.arrayItemType.type === 'array' && !isCollapsed && !collapsedFields.has(field.arrayItemType.id) && (
            renderArrayItemTree(field.arrayItemType, [...parentPath, field.id, 'arrayItem'], level + 1)
          )}
          ...
          {!isCollapsed && (
            <>
              ...
              {field.children && field.children.length > 0 && renderTreeItems(field.children, [...parentPath, field.id], level + 1)}
            </>
          )}
        </Box>
      );
  ...
  return (
    ...
    >
      ...
      <Dialog open={templateDialogOpen} onClose={() => setTemplateDialogOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" gutterBottom>
            Select a pre-built template to get started quickly. You can customize the fields after applying the template.
          </Typography>
      ...
      >
          ...
          {selectedField && (
            <Box sx={{ pt: 1 }}>
              ...
              <TextField
                ...
                onChange={(e) => {
                  const updatedField = { ...selectedField, description: e.target.value };
                  updateField(updatedField);
                  setSelectedField(updatedField);
                }}
                margin="normal"
                ...
                control={
                  <Switch
                    ...
                    onChange={(e) => {
                      const updatedField = {
                        ...selectedField,
                        logic: { ...selectedField.logic, required: e.target.checked }
                      };
                      updateField(updatedField);
                      setSelectedField(updatedField);
                    }}
              ...
              <Accordion sx={{ mt: 2 }}>
                ...
                <AccordionDetails>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    ...
                    {(selectedField.type === 'text' || selectedField.type === 'email' || selectedField.type === 'url') && (
                      <>
                    ...
                    {(selectedField.type === 'text' || selectedField.type === 'number' || selectedField.type === 'email') && (
                      <>
                        <Divider sx={{ my: 1 }} />
                        ...
                        <TextField
                          ...
                          onChange={(e) => {
                            const updatedField = {
                              ...
                              logic: {
                                ...selectedField.logic,
                                enum: e.target.value ? e.target.value.split(',').map(v => v.trim()).filter(v => v) : undefined,
                              }
                            };
                            updateField(updatedField);
                            setSelectedField(updatedField);
        ...
        <DialogActions>
          <Button onClick={() => setFieldPropertiesDialogOpen(false)}>Close</Button>
          {selectedField && (
            <Button 
              color="error" 
              onClick={() => {
